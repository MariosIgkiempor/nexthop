...
function Logo({ src, left, top, hover }: { src: string; left: number; top: number; hover: { x: number; y: number; rotate: number; delay: number } }) {
    return (
        <motion.img
            variants={{
    ...
    return (
        <div aria-hidden="true" className="relative h-full overflow-hidden">
            ...
            <div className="absolute left-1/2 h-full w-[26rem] -translate-x-1/2">
                <MainLogo />
                <Logo src="/logo-cluster/career-builder.svg" left={360} top={144} hover={{ x: 6, y: 1, rotate: 5, delay: 0.38 }} />
                ...
                <Logo src="/logo-cluster/we-work-remotely.svg" left={96} top={176} hover={{ x: -3, y: 5, rotate: 3, delay: 0.15 }} />
            </div>
        </div>
    );
}
